%% ONE-RING MODEL : EFFECTIVE RANK, COHERENCE AND NMSE
clc;
clear;
close all;
...
for j = 1:length(angular_spreads)
    ...
    nmse_all = zeros(Num_sim,1); % FIXED: Moved outside the inner simulation loop
    ...
    for i = 1:Num_sim
        ...
        %% ----------------------------------------------------
        % Sampling Binary Search for Minimum Pilot Ratio
        %% ----------------------------------------------------
        low_ratio = 0.05;
        high_ratio = 0.80;
        max_search_iter = 12;
        ...
        nmse_all(i) = mean(trial_nmse_list); % FIXED: Tracking actual NMSE metrics
    end
    %% --------------------------------------------------------
    % Average Results
    %% --------------------------------------------------------
    ...
    avg_nmse(j) = 10*log10(mean(nmse_all) + eps); % FIXED: Corrected syntax error
end
...
for j = 1:length(sv_clusters)
    ...
    for sim = 1:Num_sim
        ...
        nmse_all_sv(sim) = mean(trial_nmse_list); % FIXED: Tracking actual NMSE metrics
    end
...
for j = 1:length(rho_values)
    ...
    for sim = 1:Num_sim
        [H_exp,Rr,Rt] = generate_low_rank_mmwave_channel(Nr,Nt,L,sigma_phi,fc,d_exp,rho_t,rho_r);
        ...
        nmse_all_exp(sim) = mean(trial_nmse_list); % FIXED: Tracking actual NMSE metrics
    end
    ...
    for sim = 1:Num_sim
        ...
        switch current_case
            case 1
                for r = 1:Nr
                    for t = 1:Nt
                        if r == t, Omega_tilde(r,t) = 0.9; else, Omega_tilde(r,t) = 0.01; end
                    end
                end
            case 2
                for r = 1:Nr
                    for t = 1:Nt
                        if r == t, Omega_tilde(r,t) = 0.1; else, Omega_tilde(r,t) = 0.01; end
                    end
                end
            case 3
                for r = 1:Nr
                    for t = 1:Nt
                        if r == (Nt - t + 1), Omega_tilde(r,t) = 0.9; else, Omega_tilde(r,t) = 0.01; end
                    end
                end
            case 4
                for r=1:Nr
                    for t=1:Nt
                        if r~=t && abs(r-t) <= 2, Omega_tilde(r,t)= 0.1; else, Omega_tilde(r,t)= 0.01; end
                    end
                end
        end
        ...
        nmse_all_weich(sim) = mean(trial_nmse_list); % FIXED: Tracking actual NMSE metrics
    end
